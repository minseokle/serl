...
from indy7_env.camera.rs_capture import RSCapture
from indy7_env.utils.rotations import euler_2_quat, quat_2_euler
...
class Indy7Env(gym.Env):
    ...
    def init_cameras(self, name_serial_dict=None, name_usb_dict=None):
        ...
        for cam_name, cam_usb in name_usb_dict.items():
            cap = VideoCapture(
                VideoCapture(cap=cv2.VideoCapture(cam_usb))
            )
            self.cap[cam_name] = cap
